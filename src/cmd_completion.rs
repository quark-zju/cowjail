...
const BASH_COMPLETION: &str = r#"_cowjail_complete() {
  ...
  if [[ $COMP_CWORD -eq 1 ]]; then
    COMPREPLY=( $(compgen -W "completion profile help add list show rm run flush _mount _flush _fuse _suid" -- "$cur") )
    return
  fi

  if [[ "$cmd1" == "help" && $COMP_CWORD -eq 2 ]]; then
    COMPREPLY=( $(compgen -W "profile completion add list show rm run flush _mount _flush _fuse _suid" -- "$cur") )
    return
  fi
  ...
  if [[ "$cmd1" == "show" || "$cmd1" == "rm" || "$cmd1" == "flush" ]]; then
  ...
  case "$cmd1" in
    ...
    flush) COMPREPLY=( $(compgen -W "--name --profile -n --dry-run -v --verbose" -- "$cur") ) ;;
    ...
    _flush) COMPREPLY=( $(compgen -W "--record --profile -n --dry-run -v --verbose" -- "$cur") ) ;;
    _fuse) COMPREPLY=( $(compgen -W "--profile --record --mountpoint --pid-path -v --verbose" -- "$cur") ) ;;
