...
- The same implicit-ancestor traversal rule applies when the explicit ancestor
  action is `deny`; traversal to explicitly allowed descendants still works, and
  unmatched siblings remain hidden.
